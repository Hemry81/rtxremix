...
class lazy_rtx_remix_companion:
    ...
    def select_source(self):
        ...
        if new_folder:
            required_files = {
                "build_names.txt", "d3d8to9.dll", "d3d9.dll", "dxvk.conf", "LICENSE.txt",
                "NvRemixLauncher32.exe", "ThirdPartyLicenses-bridge.txt",
                "ThirdPartyLicenses-d3d8to9.txt", "ThirdPartyLicenses-dxvk.txt", ".trex"
            }
    
            actual_files = set(os.listdir(new_folder))
    ...
    def load_versions_from_file(self, directory):
        ...
        try:
            ...
            self.status_right.config(text="build_names.txt not found in the directory.")
            return versions
